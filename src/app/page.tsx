...
interface SectionConfig {
  id: string;
  type: 'markdown' | 'publications' | 'list' | 'internship';
  title?: string;
  source?: string;
  filter?: string;
  limit?: number;
...
function processSections(sections: SectionConfig[], locale?: string): SectionConfig[] {
  return sections.map((section: SectionConfig) => {
    switch (section.type) {
      ...
      case 'internship': {
        const internshipData = section.source ? getTomlContent<{ internships: InternshipItem[] }>(section.source, locale) : null;
        return {
          ...section,
          internshipItems: internshipData?.internships || [],
        };
      }
      default:
        return section;
